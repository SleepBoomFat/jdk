...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    //默认是RUNNING状态，线程个数为0
    //ctl 是个 Integer 的原子变量用来记录线程池状态 和 线程池中线程个数
     ...
     * 存活时间。如果当前线程池中的线程数量比核心线程数量要多，并且是闲置状态的话，这些闲置的线程能存活的最大时间。
     */
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (;;) {
            ...
            for (;;) {
                ...
                //cas失败，检测线程池状态是否变化，如果变化了，则重新跳到外层循环重新执行
    ...
    private void processWorkerExit(Worker w, boolean completedAbruptly) {
        ...
        //如果当前是shutdonw状态并且工作队列为空或者当前是stop状态当前线程池里面没有活动线程，尝试设置线程池状态为TERMINATED
        tryTerminate();
        int c = ctl.get();
    ...
    private Runnable getTask() {
        ...
        for (;;) {
            ...
            // Are workers subject to culling?   如果是核心线程 ，则take阻塞
    ...
    final void runWorker(Worker w) {
        ...
        w.firstTask = null;
        w.unlock(); // allow interrupts status = 0 允许中断
    ...
    public void execute(Runnable command) {
        ...
        //获取当前线程池的状态 + 线程数 的组合值
        int c = ctl.get();
        //如果当前线程池的线程数量<核心线程数，开启新线程
        if (workerCountOf(c) < corePoolSize) {
            if (addWorker(command, true))
                return;
            c = ctl.get();
        }
        //超过核心线程数  如果线程池是runinng状态，将任务加入队列
    ...
    public void shutdown() {
        ...
        try {
            ...
            onShutdown(); // hook for ScheduledThreadPoolExecutor 执行优雅关闭
        } finally {
            mainLock.unlock();
        }
        tryTerminate();
    ...
    public List<Runnable> shutdownNow() {
        ...
        try {
            ...
            interruptWorkers();
            tasks = drainQueue();//直接清空队列，并且返回被停止的任务
        } finally {
            mainLock.unlock();
        }
        tryTerminate();
